(function () {
  'use strict';
  /** Biometria: validação 100% on-device via Capacitor — @author Rickson.Hirata */
  ...
  function goAfterSetup() {
    ...
    if (from === 'logout') {
      ...
      if (typeof FTSession !== 'undefined' && FTSession.logout) {
        FTSession.logout().then(() => window.location.replace(FTRoutes.login))
                         .catch(()  => window.location.replace(FTRoutes.login));
      } else {
