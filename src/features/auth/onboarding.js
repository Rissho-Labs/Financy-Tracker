(function () {
  ...
  if (typeof FTSession !== 'undefined') {
    if (!FTSession.isLoggedIn()) {
      window.location.replace(FTRoutes.register);
      return;
    }
...
$('ob-next-btn').addEventListener('click', async (e) => {
  ...
  if (state.currentStep === 1) {
    ...
    const aiCheck = $('ob-ai-consent');
    const aiErr = $('ob-ai-consent-error');
    if (aiCheck && aiCheck.checked && typeof FTAiConsent !== 'undefined') {
      FTAiConsent.setConsent(true);
      if (aiErr) { aiErr.textContent = ''; aiErr.classList.remove('visible'); }
    } else if (aiCheck && !aiCheck.checked) {
      if (typeof FTAiConsent !== 'undefined') FTAiConsent.setConsent(false);
    }
    if (state.motivation === 'goal') {
      goToStep(3);
    } else {
...
async function finishOnboarding() {
  ...
  window.location.href = FTRoutes.home;
}
