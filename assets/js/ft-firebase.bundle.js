...
This typically indicates that your device does not have a healthy Internet connection at the moment. The client will operate in offline mode until it is able to successfully connect to the backend.`;
  ...
  function mapAuthError(err) {
    ...
    const map = {
      ...
      "auth/google-credential-failed": "Google autorizou, mas o app n\xE3o sincronizou. Rode npm run cap:sync e reinstale o app.",
      "auth/email-already-in-use": "Este e-mail j\xE1 est\xE1 cadastrado.",
      "auth/weak-password": "Senha muito fraca (m\xEDnimo 6 caracteres).",
      "auth/too-many-requests": "Muitas tentativas. Tente mais tarde.",
      "auth/network-request-failed": "Sem conex\xE3o. Verifique a internet.",
      "auth/popup-closed-by-user": "Login com Google cancelado.",
      "auth/popup-blocked": "N\xE3o foi poss\xEDvel abrir a janela do Google. Tente de novo.",
      "auth/cancelled-popup-request": "Aguarde e tente novamente.",
      "auth/account-exists-with-different-credential": "Este e-mail j\xE1 est\xE1 cadastrado com outro m\xE9todo (ex.: senha).",
      "auth/missing-google-client-id": "Login Google no celular precisa do ID do cliente Web no firebase-config.js (Firebase \u2192 Authentication \u2192 Google).",
      "auth/unauthorized-domain": "Dom\xEDnio n\xE3o autorizado. No Firebase, adicione localhost em Authentication \u2192 Settings \u2192 Authorized domains.",
      "auth/operation-not-allowed": "Login com Google desativado. Ative o provedor Google no Firebase Authentication.",
      "auth/internal-error": "Erro interno. Tente de novo em instantes.",
      "auth/web-storage-unsupported": "Armazenamento bloqueado no app. Atualize o WebView ou reinstale o app.",
      "auth/firebase-not-ready": "Firebase n\xE3o carregou. Reinstale o app ou verifique a internet.",
      "permission-denied": "Sem permiss\xE3o no Firestore. No Console \u2192 Firestore \u2192 Regras, publique o arquivo firestore.rules e confira se o banco \xE9 (default).",
      "auth/native-google-failed": "N\xE3o foi poss\xEDvel abrir o Google no app. Confira SHA-1 no Firebase, rode npm run cap:sync e reinstale.",
      "auth/native-google-no-token": "Google n\xE3o devolveu token. Verifique SHA-1 e google-services.json no Firebase."
    };
    if (!code && err && err.message) {
      ...
      if (msg === "gsi_load_failed") {
        return "N\xE3o foi poss\xEDvel carregar o Google. Verifique a internet.";
      }
  ...
  function packGoogleResult(result) {
    ...
    const googleCred = result.credential && result.credential.idToken ? {
          ...
          var client = window.google.accounts.oauth2.initTokenClient({
            ...
            callback: async function(tokenResponse) {
              if (tokenResponse.error) {
                ...
                err.code = tokenResponse.error === "access_denied" ? "auth/popup-closed-by-user" : "auth/" + tokenResponse.error;
                finish(reject, err);
                return;
              }
              try {
  ...
  async function signInWithGoogleNativeBridge() {
    const api2 = await waitForNativeGoogleApi();
    if (!api2) return null;
    const native = await api2.signIn();
    const cred = GoogleAuthProvider.credential(native.idToken, native.accessToken || void 0);
    let result;
    try {
  ...
  async function signInWithGoogle() {
    ...
    if (isNativeCapacitor()) {
      try {
        ...
        if (nativeCode === "auth/popup-closed-by-user" || nativeCode === "auth/cancelled" || nativeErr && nativeErr.message && String(nativeErr.message).includes("cancel")) {
          throw nativeErr;
        }
      }
      if (webClientId) {
  ...
  async function ensureAuthForFirestore(expectedUid) {
    const user = await waitForAuthUser(8e3);
    if (!user) {
  ...
  async function isEmailRegistered(email, exceptUid) {
    ...
    if (!methods || methods.length === 0) {
      const uid2 = await findUidByEmail(mail).catch(function() {
        return null;
      });
      if (!uid2) return false;
      return exceptUid ? String(uid2) !== String(exceptUid) : true;
    }
    if (!exceptUid) return true;
    const cur = auth.currentUser;
    if (cur && cur.uid === exceptUid && cur.email && cur.email.toLowerCase() === mail.toLowerCase()) {
      return false;
    }
